...
export const VendorAccountSettings: React.FC = () => {
  ...
  const displayName = profileAny?.fullName ?? profileAny?.username ?? 'Vendor Owner';
  const phoneNumber = profileAny?.phoneNo ?? profileAny?.phone ?? '';
  const accountType = profileAny?.accountType ?? 'individual';
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans antialiased pb-24">
      <CustomSettingsHeader
        ...
        subtitle="Manage your vendor profile, organization, payout and preferences"
      />
        ...
        <div className="w-full bg-white rounded-2xl p-4 border border-border/50 shadow-sm flex items-center gap-4 text-left">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="inline-flex items-center gap-1 mt-1.5 text-[11px] font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-full capitalize">
              <CheckCircle className="w-3 h-3 fill-current" /> {accountType} vendor
            </span>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl border border-border/50 shadow-sm overflow-hidden divide-y divide-border/50">
            <MenuItem
              ...
              onClick={() => navigate('/vendor/settings/personal')}
            />
            {accountType === 'organisation' && (
              <MenuItem
                icon={Building2}
                title="Organization Info"
                subtitle="Manage company name, registration and tax details"
                onClick={() => navigate('/vendor/settings/organization')}
              />
            )}
            <MenuItem
              icon={FileText}
              title="KYC & Documents"
              subtitle="View identity and business compliance documents"
              onClick={() => navigate('/vendor/settings/documents')}
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl border border-border/50 shadow-sm overflow-hidden divide-y divide-border/50">
            <MenuItem
              ...
              onClick={() => navigate('/vendor/settings/payout')}
            />
          </div>
        </div>
