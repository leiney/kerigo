...
const createCustomIcon = (color: string) => {
  const html = `
    <div style="position: relative; width: 36px; height: 42px;">
      <div style="position: absolute; left: 50%; top: 0; transform: translateX(-50%); width: 36px; height: 36px; background: ${color}; border-radius: 50%; border: 3px solid white; box-shadow: 0 10px 20px rgba(0,0,0,0.18);"></div>
      <div style="position: absolute; left: 50%; bottom: 0; transform: translateX(-50%); width: 10px; height: 10px; background: ${color}; border-radius: 50%; border: 2px solid white;"></div>
    </div>
  `;
  return L.divIcon({
    html,
...
const formatOrderItems = (items: unknown, fallbackCount?: number): string => {
  ...
  if (items && typeof items === 'object') {
    if ('quantity' in items && typeof items.quantity === 'number' && items.quantity > 0) {
      return `${items.quantity} item${items.quantity === 1 ? '' : 's'}`;
    }
    return fallbackCount != null && fallbackCount > 0 ? `${fallbackCount} item${fallbackCount === 1 ? '' : 's'}` : '';
  }
...
export const TrackOrder: React.FC = () => {
  ...
  const storeLat = order?.extraData?.vendor?.location?.latitude;
  const storeLng = order?.extraData?.vendor?.location?.longitude;

  const riderCoords = extractLatestRiderCoords(order);
  const riderLat = riderCoords.lat;
  const riderLng = riderCoords.lng;

  const destLat = order?.extraData?.location?.latitude;
  const destLng = order?.extraData?.location?.longitude;

  const store = {
    name: order?.extraData?.vendor?.name ?? 'Store',
    description: order?.extraData?.vendor?.location?.address ?? 'Vendor pickup location',
    lat: typeof storeLat === 'number' ? storeLat : 0,
    lng: typeof storeLng === 'number' ? storeLng : 0,
  };

  const riderName = order?.extraData?.rider?.fullName ?? order?.extraData?.rider?.name ?? '';
  const riderPhone = order?.extraData?.rider?.phoneNo ?? null;
  const riderPickupTime = order?.extraData?.rider?.estimatedPickupTime ?? null;
  const rider = {
    name: riderName,
    phone: riderPhone,
    pickupTime: typeof riderPickupTime === 'number' ? riderPickupTime : null,
    lat: riderLat,
    lng: riderLng,
  };

  const deliveryLocation = {
    label: order?.extraData?.location?.address ? 'Delivery Address' : 'Customer Address',
    description: order?.extraData?.location?.address ?? 'Customer delivery location',
    lat: typeof destLat === 'number' ? destLat : 0,
    lng: typeof destLng === 'number' ? destLng : 0,
  };

  const currentStepKey = mapStatusToStepKey(getLatestStatus(order));
  ...
  const riderRoute = getRiderRouteCoordinates(order);
  ...
  if (deliveryLocation.lat !== 0 && deliveryLocation.lng !== 0) {
    routeCoordinates.push([deliveryLocation.lat, deliveryLocation.lng]);
  }

  const orderStatus = formatStatus(getLatestStatus(order));
  ...
  const itemCount = Array.isArray(order?.orderItems)
    ...
    ? order.itemCount
    : undefined;
  const orderItems = formatOrderItems(order?.orderItems ?? order?.items, itemCount);
  const orderTotal = typeof order?.total === 'number' ? `KES ${order.total.toLocaleString()}` : order?.total ?? '';
  const orderSteps = Array.isArray(order?.steps) && order.steps.length ? order.steps : buildFallbackOrderSteps(order);

  const mapBounds = routeCoordinates.length ? L.latLngBounds(routeCoordinates as [number, number][]) : null;

  return (
    <PullToRefresh onRefresh={refreshOrder}>
      <div className="min-h-screen bg-background text-foreground font-sans antialiased pb-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <div className="flex items-center gap-2">
                ...
                {(() => {
                  const badgeStyles = getStatusBadgeStyles(orderStatus);
                  return (
              ...
              <p className="text-[11px] text-foreground/50 mt-0.5">Order #{order?.orderNo ?? '—'}</p>
            </div>
          </div>
          <button
        ...
        <div className="px-2.5 space-y-2.5">
          {!hasOrder ? (
            <div className="bg-white rounded-3xl p-6 border border-border/50 shadow-sm text-center">
              <p className="text-sm font-semibold text-foreground">Order data unavailable</p>
              <p className="mt-2 text-xs text-foreground/60">
                Pull down to retry or go back and select another order.
              </p>
              <div className="mt-4">
                <Button
                  variant="outline"
                  size="sm"
                  className="text-primary border-primary/20 hover:bg-primary/5 h-10 px-4 text-[12px] font-semibold"
                  onClick={() => navigate(-1)}
                >
                  Back to orders
                </Button>
          ...
          <motion.div
            ...
            className="relative rounded-2xl overflow-hidden border border-border/50 shadow-sm h-105"
          >
            {routeCoordinates.length ? (
              ...
              >
                ...
                {store.lat !== 0 && store.lng !== 0 && (
                  <Marker position={[store.lat, store.lng]} icon={createCustomIcon('#3f940e')}>
                    <Popup>
                      <div className="p-2">
                ...
                {rider.lat !== 0 && rider.lng !== 0 && (
                  <Marker position={[rider.lat, rider.lng]} icon={createCustomIcon('#3f940e')}>
                    <Popup>
                      <div className="p-2">
                        <p className="font-bold text-sm">Rider location</p>
                        <p className="text-xs text-gray-600">Live rider position</p>
                      </div>
                    </Popup>
                  </Marker>
                )}
                ...
                {deliveryLocation.lat !== 0 && deliveryLocation.lng !== 0 && (
                  <Marker position={[deliveryLocation.lat, deliveryLocation.lng]} icon={createCustomIcon('#3b82f6')}>
                    <Popup>
                      <div className="p-2">
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-sm text-foreground">
                    {rider.name || 'No rider assigned'}
                  </p>
                  <p className="text-xs text-foreground/50">
                    {rider.name ? 'Assigned rider' : 'Rider details unavailable'}
                  </p>
                  {rider.phone ? (
                    <p className="text-[11px] text-foreground/70 mt-1">Phone: {rider.phone}</p>
                  ) : (
                    <p className="text-[11px] text-foreground/50 mt-1">Phone unavailable</p>
                  )}
                </div>
              </div>
              ...
              {rider.phone ? (
                ...
                <div className="text-xs text-foreground/50">No rider contact available</div>
              )}
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button
                onClick={() => navigate(`/customer/orders/${order?.orderID ?? order?.id ?? resolvedOrderId}`)}
            ...
            <div className="flex items-center justify-between">
              <p className="text-xs text-foreground/60">{orderItems || 'No order items available'}</p>
              <p className="text-sm font-bold text-foreground">{orderTotal || 'Total unavailable'}</p>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-primary">Your order is safe with us</p>
                <p className="text-xs text-foreground/50 mt-0.5">We'll notify you when it's delivered</p>
              </div>
            </div>
